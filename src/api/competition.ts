...
// 타이머 정지 등 실제 동작은 클라이언트에서 처리하고, 서버에는 어느 매치의 상태인지만 기록한다.
export const pauseBotMatch = async (
  matchId: number,
  botPause: boolean,
): Promise<BotPauseResponse> => {
  const res = await client.get('/bot/pause', { params: { matchId, botPause } });
  return res.data;
};